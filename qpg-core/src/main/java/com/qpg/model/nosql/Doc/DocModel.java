...
public class DocModel extends NoSQLDataUnit {
  ...
  public void map(QPGNode root, List<String> agg_labels) {
    ...
    String prefix = "";
    mapNode(root, agg_labels, this, prefix);
  ...
  private void mapNode(QPGNode node, List<String > agg_labels, DocModel doc, String prefix){
    ...
    for(UMLAttribute attr: attrs){
      if(keys.contains(attr))
        continue;
      doc.addField(new DocField(node.getUmlClass().getName(), attr, prefix));
    }
    ...
    for(QPGEdge edge: node.getOutEdges()){
      if(edge.isInAgg(agg_labels)){
        if(edge.getCard(edge.getEndNode()) > 1){
          ...
          DocField f = new DocField(edge.getEndNode().getUmlClass().getName(), doc_child, true, new_prefix);
          doc.addField(f);
