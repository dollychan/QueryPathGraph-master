...
public class CassandraClient extends DBClient {
  ...
  public Status read(ExecPlan.QueryExecPlan query,
    ...
    if(!eas.isEmpty() || !ieas.isEmpty()) {
      ...
      for(String[] a: ieas){
        if(values.get(a[0]) == null){
          where = where.and(QueryBuilder.gt(a[0], query.getRandomQueryValue(a[0])));
          continue;
        }
        //if(a[1].equalsIgnoreCase(">="))
        where = where.and(QueryBuilder.gt(a[0], values.get(a[0])));
      }
    }
  ...
  public void runEachUpdatePlan(String test_name,
    ...
    for(ExecPlan.UpdateExecPlan u: updates){
      ...
      if(u.getQueryEAs() != null){
        read( u.getClass_name(), u.getQueryEAs(), values,results);
      }
      for(Row r : results){
        List<ColumnMetadata> columns = ksmetadata.getTable(u.getClass_name()).getColumns();
        ...
        insert(u.getClass_name(), new_values);
      }
    }
  }
